...
def webpage():
    ...
    interval_time = interval_timer(prev_time)
    ...
    <br>
    <h1>Interval Timer Display</h1><br>
    <p><button class="button" name="INTERVAL" value="INTERVAL" type="submit">INTERVAL TIMER</button></a></p>
    <p>Time since last clicked {interval_time}</p>
    </body>
    <script>
    // LED buttons handler
...
def buttonpress(request: HTTPRequest):
    ...
    if "INTERVAL" in query:
        ...
        with HTTPResponse(request, content_type=MIMEType.TYPE_HTML) as response:
            response.send(str(interval_time))
